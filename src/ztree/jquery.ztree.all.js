  ...
  var settings = {},
    ...
    data = {
      addNodeCache: function (setting, node) {
        // console.log('addNodeCache: ', setting, node);
    ...
    view = {
      ...
      makeDOMNodeIcon: function (html, setting, node) {
        ...
        const svgIcon = node.icon
          ? `<svg id="${node.tId}${consts.id.ICON}" title="treeNode${consts.id.ICON}" class="${view.makeNodeIcoClass(
              setting,
              node
            )}" style="${view.makeNodeIcoStyle(setting, node)}"><use :xlink:href="${node.icon}" /></svg>`
          : '';
        html.push(
          `${svgIcon} <span id="${node.tId} ${consts.id.SPAN}" class="${consts.className.NAME}">${name}</span>`
        );
        // html.push(
        //   "<span id='",
        //   node.tId,
